  ...
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    ...
    <div class="flex items-center space-x-2 mb-2">
      <svg width="40" height="40" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg">
        <circle cx="20" cy="20" r="18" :fill="getRandomColor()" />
      ...
      <span class="text-[18px] font-medium">{{ row.kota }}</span>
    </div>
    <div class="text-[34px] font-bold text-gray-800 text-right">{{ Number(row.rata_rata_progress).toFixed(0) }}%</div>
    <div class="text-[11px] font-bold text-[#92A1AD] text-right">{{row.created_at}}</div>
    <div class="w-full bg-gray-200 rounded-full h-2 mt-2">
      <div class="bg-indigo-500 h-2 rounded-full" :style="{ width: row.rata_rata_progress + '%' }"></div>
    </div>
  </div>
          </div>
        </div>
        ...
        <div v-if="!data || data.length === 0" class="text-center py-8">
          <div class="text-gray-500 dark:text-gray-400">
            ...
            <p>Tidak ada Kota yang tersedia</p>
          </div>
        </div>
      </div>
    </div>
