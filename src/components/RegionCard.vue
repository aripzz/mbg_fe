<template>
  <div class="bg-white rounded-lg p-4 shadow-sm">
    <div class="flex items-center space-x-2 mb-2">
      ...
      <span class="text-[18px] font-medium">{{ region.name }}</span>
    </div>
    <div class="text-[34px] font-bold text-gray-800 text-right">{{ region.percentage }}%</div>
    <div class="text-[11px] font-bold text-[#92A1AD] text-right">Diupdate 05 Juli 2025</div>
    <div class="w-full bg-gray-200 rounded-full h-2 mt-2">
      <div :class="progressColor" class="h-2 rounded-full" :style="{ width: region.percentage + '%' }"></div>
    </div>
  </div>
</template>
  ...
  computed: {
    iconColor() {
      const colors = {
        ...
        'Surabaya': '#a855f7', // purple-500
        'Magelang': '#3b82f6', // blue-500
        'Kupang': '#eab308' // yellow-500
      }
      return colors[this.region.name] || '#6b7280' // gray-500
    },
    progressColor() {
      const colors = {
        'Solo': 'bg-green-500',
        'Surabaya': 'bg-purple-500',
        'Magelang': 'bg-blue-500',
        'Kupang': 'bg-yellow-500'
      }
      return colors[this.region.name] || 'bg-gray-500'
    }
  }
}
</script>
